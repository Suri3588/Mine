...
if [ "$1" == "--help" ]; then
  echo -e "usage: \x1b[92msource\x1b[0m $0 [vars-file] [--env=environment] [--clean]"
  echo
  ...
  echo "if 'nameInValue' is '-env', this will lookup 'terraform_variable-environment'"
  echo "Otherwise, the nameInVault is used 'as is' to lookup the secret"
  echo
  echo "These variables and the secrets are written to 'secrets.auto.tfvars' file in the current directory."
  echo
  ...
  echo
  echo "az keyvault secret set --name XYZ --vault-name NucleusIaCVault --file whatever"
  echo 
  ...
  echo
  echo "--env=environment will set the name of the environment. If not specified, it is inferred from the current"
  echo "directory name"
  echo
...
vaultName="NucleusIaCVault"

if [[ -z $environ ]]; then
  environ=$(basename `pwd`)
  echo "Inferring enviroment as $environ"
fi
...
#export ARM_ACCESS_KEY=$(az keyvault secret show --name TerraformStorageAccessKey-$1 --vault-name NucleusIaCVault --query value -o tsv)