...
if [ $? -ne 0 ]; then
  kubectl --namespace=$NAMESPACE create secret generic alerts --from-literal=sendmailAPIKey="$sendmailAPIKey" --from-literal=pagerDutyKeyChunkCount="$pagerDutyKeyChunkCount"
...
renderElastalertRule "$IMPORTANT_RULE_TYPE" "chunkCount-3K.yaml" "$pagerDutyKeyChunkCount"
createElastalertRules
